...
\begin{document}
    ...
    The Overmare's Journal is the Game Master's Guide to handling the interaction between the characters and the Equestrian Wasteland. The \textbf{Fallout Equestria: Wasteland Ventures} Core Rulebook has the necessary info players - and you - need to know how to play. 
    ...
    \emph{The Overmare's Journal} is more of a reference guide than a novel - hence, skipping things and flipping through the pages as needed is a valid way of reading this document, as is browsing from start to finish. The topics of the book are listed in order of how a normal campaign and session would develop from a spark of an idea to the last minutes of a session. Finally, the book lists more in-depth lore of the Equestrian Wasteland and beyond, hopefully providing ample inspiration for a story of your and your friends' creation.
    
    \section*{Notes to GM-ing}
    \addcontentsline{toc}{section}{Notes to GM-ing}
    
    The first and notably overriding rule of any game for any GM, is to remember that the game is supposed to be fun for all - players and GM included. For players, they seek entertainment from the game, and game must provide such for the players to return for the next session. GM should try to be lenient and adaptive where possible, capable of overlooking or altering rules for the group benefit. In the end, the game is a collaborative effort to having fun.
    
    Secondly, as the GM you have the final say in the matter of rules. You are free to make a ruling on the fly without checking from the rules, as long as players have fun and the story progresses. In-depth checking of the rules can be done after the session or during a break.
   
   \chapter{Preparing a Campaign}
   
   \begin{quote}
   	\emph{Okay, grab your pencils, sheets, snacks, dice and let's get this game started! ...Where did I put my notes?}
   	
   	\emph{-Gaffer, the local GM of Ogres and Oubliettes from Junction Town}
   \end{quote}

	All stories have a beginning, and it is your task as a Game Master to provide said beginning. Typically games are run as campaigns, spanning multiple sessions with an overarching story, or "one-shots", condensed adventures that begin and end within a single session. 
	
	Campaigns, as mentioned, are multi-session games, typically with an overarching narrative that combines the sessions and their associated adventures together. 
	
	\chapter{Running a Session}
	
	\section*{Encounters}
	\addcontentsline{toc}{section}{Encounters}
...
% Fuck this mentality, it burned me out ~Waak
%    The undeniable, all else overriding rule of any game for any GM, is to remember that the game is and will always be for the players. Players seek entertainment from the game, and game must provide such for the players to return for the next session. GM should be lenient and adaptive, capable of overlooking or altering rules for the group benefit. GM must show initiative for unpredictable events players might bring forth, respect the shortcomings of their actions, be able to reward ingenious or insane plans and their successes, and present politely the aftereffects of players' actions, and their impact to the game.
    ...
    As a whole, the Ministries were armed with good intentions; and a road to Tartarus is paved with good intentions. However, we stress that you do not imagine that the Ministries were run by mustache-twirling villains, but rather by flawed individuals.
    
    Now, it doesn't mean that the Ministries were full of nice ponies either, but by and large they wanted to do what was necessary to protect Equestria as a nation in a situation the country had largely never been before. War was very much a new, exhausting experience for the ponykind, and the ministries were an effort to mitigate. But it would be a lie to say that all personnel in the ministries were selfless or without their own ends.
    
    In the Wasteland, the ministries' buildings lie in ruins and often filled with mutants, especially those that belong to MAS. Few remember, and even less care, about the Ministries' legacy, but that doesn't mean you cannot give your players some insight into them. 
    
    Ministries would have a near legendary status among both scavengers and Steel Rangers, being treasure troves of old-world tech, \textit{especially} the dangerous kind.
    ...
    \textit{Ministry Mare Twilight Sparkle:} A lavender-coated unicorn mare and Princess Celestia's former student who sought to bring an end to the long-lasting war with the zebra as quickly as possible, even if the ethics of those methods skewed. Values her friends, even as they fell apart and was prone to panic and over-organization; think check-up lists and slight micromanagement. Last seen in a MAS hub in Maripony.
    ...
    \textit{Ministry Mare Rarity:} Rarity was a fashionista and a seamstress with her own boutique before the war, and during it was granted the directory role of MoI, perhaps the most noticed and in the forefront aside from Ministry of Morale. In an act of desperation, Rarity began to take on black magic from Black Book, a zebra artifact. 
    
    In a last ditch effort to bring her friends on speaking terms, she created the Ministry Mare statuettes by cutting up her soul and attaching a piece into each, one of each for every Ministry Mare. These statuettes can be found in the Wasteland, often in important plot milestones as rewards. Her last wereabouts are in Canterlot.
    ...
    \textit{Ministry Mare Pinkie Pie:}
...
%    ●	Where happens 
%    ●	When happens 
%    ●	Who are participating
%    ●	What happens if taken part of
%    ●	What happens if ignored
%    ●	Does it recur
%    ●	Difficulty to succeed
%    ●	Difficulty to fail
%    ●	Reward for success
%    ●	Penalty for failure
...
%    \subsection*{Pony}
    ...
    Likewise, some NPCs are more difficult to convince than others, or the character's claim is so unbelievable it's difficult to sell as true. Cynics in particular can be difficult to convince, while naïve characters believe far more outrageous lies than most folk would.
    ...
    %TODO: Reorganize Crit Fail Tables to be more readable and preferrably on one page
...
%    Roll (1d10)	Effect
    ...
    \section*{Scavenging, Invention and Alchemy}
    ...
    This part also includes searching for herbs, and includes tables for both with and without Keen Eye -perk for alchemy.
    \vfill
    \textbf{NOTE: If the player rolls a critical success when rolling LCK to scavenge a kitchen or bedroom, they find a hidden Safe in either location.}
 	\vfill
    \textbf{NOTE: If the player has a Critical Success when rolling LCK for scavenging in a garage or basement, they find a hidden stash of Caps of 5+2d10 in either location.} 
\begin{table}%[t]
        \centering
  ...
  %  Loot Table 5.6. Safe/Hidden container
        ...
        \begin{tabular}{|c|c|c|}
           ...
           \textbf{1d10}    & \textbf{Item Type}    & \textbf{Amount}  \\ 
                    ...
                    6       &  Spell Instructions   & 1          \\  
                    7		&  Alchemy Recipe		& 1			\\
                    8		&  Components			& 1+1d4 	\\
                    9		&  Melee/Unarmed weapon	& 1			\\	
                    10		&  Explosive			& 1			\\ \hline
        \end{tabular}
        \label{tab:my_label}
    \end{table}
       ...
       \begin{table}
	\centering
	\caption{Loot table: Herbs, Normal search}
	\rowcolors{1}{gray!30}{gray!5}
	\begin{tabular}{|c|c|c|}
		\hline
		\textbf{1d100}    	& \textbf{Item Type}    & \textbf{Amount}  \\ 
			1-5      		&  White Herb         	& 1d4           \\
		    6-15   			&  Black Herb         	& 1d4         \\
		    16-30   		&  Blue Herb            & 2d4          \\ 
		    31-60   		&  Red Herb   			& 2d4           \\    
		    61-100  		&  Green Herb  			& 3d4         \\ \hline
	\end{tabular}
	\label{tab:my_label}
\end{table}
...
\begin{table}
	\centering
	\caption{Loot table: Herbs, Keen Eye -perk}
	\rowcolors{1}{gray!30}{gray!5}
	\begin{tabular}{|c|c|c|}
		\hline
		\textbf{1d100}    	& \textbf{Item Type}    & \textbf{Amount}  \\ 
		1-15      			&  White Herb         	& 1d4           \\
		16-30   			&  Black Herb         	& 1d4         \\
		31-50   			&  Blue Herb            & 2d4          \\ 
		51-70   			&  Red Herb   			& 2d4           \\    
		71-100  			&  Green Herb  			& 3d4         \\ \hline
	\end{tabular}
	\label{tab:my_label}
\end{table}
...
%    6	Caps	3d10
    ...
    Most societies in the Wastes do not approve of stealing, but depending on the society itself, the repercussions may vary greatly. While some societies are willing to let a thief go with a simple fine, others may be only satisfied when they take the offending party's hoof as punishment. Or at least attempt to.
   ...
   %TODO: Update to latest tables using tabularray (tblr) element
