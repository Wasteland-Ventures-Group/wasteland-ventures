...
%	\end{center}
	...
	Your character's \textbf{Skills} reflect their various abilities and knowledge. The skill value indicates how good your character is at that skill, and at character creation you select three skills to \textbf{Tag} to mark specialization of the character. Skill values can be increased further by allocating \textbf{Skill Points} at character creation and \textbf{Level Ups}. Skill values are normally expressed by a number between 1 to 85; the higher the Skill value, the better your chance of success.
	...
	The character chooses only one spell at the begin of play. She cannot learn new spells on level up or otherwise. Instead, her Potency is twice as effective with that spell than normal.
	...
	However, her Potency on spells is half of that of normal.
...
%	\end{center}
	...
	The character is a practitioner of zebra shamanism in the way of her ancestors, and benefits from ancient spirit pacts. The character starts with \textbf{Spirit Affinity}. She has an effective +2 Spirit Affinity with a particular type of Spirit of her choice, and an effective -2 Spirit Affinity with another type of Spirit of the GM's choice. Which spirits are the favorite and unfavorite do not have to be made known before they're encountered, if so desired.
	
	The character has access to Spirit Magic part of Zebra Magic, found in the \emph{\textbf{Magic Codex}}. Details on how shamanism and Spirit Affinity works can be found under Chapter 8 - Zebra Magic - Shamanism.
	...
	In addition, \textbf{Explosives deal their full damage on the central hex of the Burst Template only} - other targets within the Burst's area take half of the damage.
	
	However, Explosive weapons do not deal critical damage, nor can they be used to do \textbf{Called Shots}. In case of a critical success, other effects may apply. In case of a failure, the explosive can deviate at least 1 hex up to half the launched/thrown distance from the intended target. In air-to-air combat, a missed attack automatically deals no damage.
	...
	Though alcohol and chems do not provide one with HP or Rads, they do give additional temporary effects to one's SPECIAL's and other effects. They do have varying chances of addiction, meaning that a character may end up with severe dependence on a drug. An addicted character will experience Withdrawal penalties unless they consume some of their preferred addiction source at least once in 24 hours. 
	
	One can cure their addiction with a dose of Fixer, a notoriously expensive chem, or beat the withdrawal naturally. After each month (30 days) without touching the substance, a character can roll END check to see if they've beaten the addiction.
	...
	Base falling damage applies to falling distances between 4 and 8 meters. If the falling distance is greater than that, the damage increases. For every 4 meters fallen beyond 8 meters, the damage is increased by 2.
	...
	Examples of falling damage can be seen on Table 5.2.
	...
	The effects of accumulated Rads can be seen in Table 5.3.
	...
	Likewise, existing potions, like a Healing potion or Healing powder can be used when creating chems, to add a similar effect to the new Chem. Chems do not have a set number of ingredients required, but the general rule is that simpler chems use less, and have more common ingredients than complex, particularly potent Chems.
	...
	\begin{verse}
		\emph{\textbf{Example:} Doc Red wants to create a Chem that heals its user both in HP and Strain. GM and him decide that Red will require a Healing Potion, 2 Blue herbs, and a ground fresh Jackalope Horn. They decide that the effect of the chem is +2 HP healed per turn for 2 Rounds and +5 Strain recovered per turn for 2 Rounds. They agree that the potent nature of the Chem means Doc Red will require 5 successful Medicine rolls instead of 4. They calculate that the addiction chance from this Chem is 20\%, with -2 Potency and -1 END as a Withdrawal effect. After 6 days, Doc Red has successfully rolled Medicine 5 times and the Chem is complete}.
	\end{verse}
	...
	If combatants roll the same total initiative, the one with the highest AGI acts first.  If the combatants' PER are identical, the one with the highest LCK acts first. If both are identical, they act simultaneously. The GM should keep track of the sequence of simultaneous actions.
...
%	\end{verse}
	...
	\begin{description}
		...
		\item[Potency] - determines the power of spells. Potency is determined by the spell school of each individual spell, combining SPECIAL value with the character's level. 
	...
	Often magic is used for offensive purposes, and thus deal damage to the intended target. Damage is listed similarly to damage done by Equipment, with base damage listed first and the number of d10 dice to be thrown written in parenthesis, i.e. 20+(3). Some spells take the caster's Potency into account as a multiplier when calculating the base damage, i.e. Potency*2+(3).
	...
	To learn a new Maneuver by self-teaching, the character must roll 4 successful checks of AGI/END/CHA -2, the stat depending on the Maneuver the character wants to learn. Only one check may be made per day, and this check can be re-rolled once per day. These successful checks do not have to be consecutive.
	...
	Shamans possess a unique secondary stat called \textbf{Spirit Affinity}, which is somewhat similar to unicorns' Potency. It indicates how powerful spirits the shaman can summon, barter with and bind. Spirit Affinity can use either CHA, INT or LCK as its linked stat.
	...
	Spirit Power rating is a number between 1 and 10. The exact effect of Spirit Power varies and are explained in future sections or in the details of each individual spirit.
	...
	If the bartering is successful, the spirit lends some of its power to shaman. If the bartering result is a tie, the spirit wins, and flees contently without providing any power. If the shaman critically fails her Barter roll, the spirit becomes hostile and attacks the shaman.
