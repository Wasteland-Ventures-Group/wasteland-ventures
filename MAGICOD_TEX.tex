...
% Example Below:
% \begin{description}
% 	\item[$<$10 Potency] Tiny Splash area
% \end{description}
\begin{quote}
	\begin{description}
		\item[$<$10 Potency] 	Tiny Splash area
		\item[10-19 Potency] 	Small Splash area
		\item[20+ Potency] 	Large Splash area
	\end{description}
\end{quote}
...
\begin{quote}
	\begin{description}
		\item[$<$10 Potency] 	Small Splash area
		\item[10-19 Potency] 	Large Splash area
		\item[20+ Potency] 	Huge Splash area
	\end{description}
\end{quote}
...
Sun Celebration is a bit more difficult of a maneuver to pull off in the often-cloudy Wasteland: As her first action, the flier must disperse clouds from a Small Splash area with a Thaumaturgy check. Dispersing the clouds costs 5 AP, and the dispersed clouds gather back after 5 turns. This action costs no Strain.
...
\begin{itemize}
	...
	\item If the Thaumaturgy roll is unsuccessful, everyone, including the Flier, in the Large Splash Area suffer a Minor Distraction.
\end{itemize}
...
The flier flies so fast that they are set on fire, after which the flier will ram their opponent, spreading the fire at any adjacent characters to the target in a Small Splash Area. Thaumaturgy -10 roll is required for successful maneuvering.
...
% Table
{
	...
	\begin{tabu} to 75mm{| X[l,4] | X[l,7] |}
		...
		Range     		& Potency*2 m 		\\
		Target      	& -					\\
		Area of Effect  & Large Splash area 	\\
...
\textbf{Ascended Spell:} When casting the spell, the caster can decide if they cast the spell normally, or as a Large Splash Area centered on caster. When cast as AoE, this spell affects any who are within the Splash Area. In addition, the targets of this spell may roll END to resist the Minor Distraction.
...
\begin{itemize}
	...
	\item The spell's Area of Effect is one category larger, to a maximum of Huge Splash area.
...
The caster conjures a dazzling display of imaginary fireworks, crackers and cherry-bombs in an area of her choosing, thus creating a distracting event for anyone within Small Splash area of the location.
...
\textbf{Ascended Spell:} The spell becomes a Tiny Splash area instead of affecting 1 target. Likewise, the Strain cost of the spell becomes 3.
...
\textbf{Ascended Spell:} Caster can modify their spell with one of two effects: The spell now inflicts a Major Distraction on the target if they fail the AGI check, or the spell now affects a Small Splash area instead of 1 target, but with Minor Distraction.
...
\textbf{Ascended Spell:} The effect of this spell becomes a Tiny Splash Area centered on caster, however the Strain cost rises to 2. Likewise, the caster is no longer required to touch the item she wants to enchant.
...
If the caster enlarges themselves during combat, the maximum they can enlarge themselves to is Large Splash Area. In this size, they will not cause targets to suffocate, due to their form not being condensed enough. Likewise, enemies gain a +10 to hit them.
...
\textbf{Ascended Spell:} The spell's Splash area is Small Splash instead of Tiny, and the spell can cause 1 target within the area to become Stunned. The target gets an Opposed roll of Caster's INT versus the target's END.
...
The caster conjures a thick copper-colored fog around a target in a Tiny Splash area. Creatures outside of the Tiny Splash area have a visibility penalty determined by the caster's Potency, while the target of the spell suffers no such penalty. If any other character moves inside the Spell's area of effect, they still suffer the visibility penalty, as if they were outside of it. The Wall of Mist moves with the target, tethered to them with arcane energy.
...
This potion-like bomb turns gaseous when the thrown container shatters, releasing rage-inducing gas on a Small Splash area. This effect lingers on the battlefield for 3 turns, causing any who come into contact with it gain Enraged- status effect, negated by an END-roll.
...
\begin{itemize}
	...
	% REPLACED BY FLAMING
	%\item \textbf{ [Weapon] Flaming enchantment:} Projectiles fired with this weapon explode on impact, dealing 20+[Spirit Power] damage to all in a Tiny Splash template.
	
	%In case of Full Auto weapons, the 20+[Spirit Power] damage is dealt in addition to each damage roll made. The initial target takes weapon damage + the additional 20+[Spirit Power] damage, and others within Tiny Splash template of the target take 20+[Spirit Power] damage only.
...
\smallskip
{
	...
	\begin{tabu} to 70mm{| X[l,4] | X[l,7] |}
		...
		Target      	& AoE           		\\
		Area of Effect  & Small Splash Area  	 \\
...
A fiery projectile that deals damage to everything in s Small Splash area, and sets the area on fire for 4 Rounds. Anyone who gets hit or steps into this fire have to roll END to resist getting a Burning- status effect.
...
%Caster teleports [Spirit Power]/2 number allies (to a minimum of 1) \textit{or} self. In combat, the allies have to be in a Tiny Splash area from the caster, and can be teleported within a Huge Splash area. Teleported allies can re-appear separate or in a group.
...
The caster sends out a wave of healing energy around them that heals everyone within Small Splash area. 
...
\begin{itemize}
	...
	\item \textbf{ [Weapon] Light enchantment:} Projectiles fired with this weapon explode on impact with a bright light, inflicting Blinded Status on all in a Tiny Splash template. Successful END check negates the effect. If not already blinded, the check must be made again for each attack.
...
\begin{itemize}
	...
	\item \textbf{ [Weapon] Arching enchantment:} A successful hit with the weapon causes an arch of electricity to form between the target and any other target within Small Splash Radius, the secondary targets taking 1+ [Spirit Power]/2 damage dice worth of damage.
...
The caster sends a ball of lightning that bounces between maximum of [Spirit Power]/2 number targets in a Large Splash area. This lightning ball can hit a same target multiple times, but not twice in a row. Each hit causes 1+(4) damage to the target.
...
\begin{itemize}
	...
	\item \textbf{ [Armor] Toxicity enchantment:} Once per day upon command, all enemies in Large Splash Template, centered on you, are poisoned for [Spirit Power]/2 of turns. Poison effect is random.
...
The caster creates a poisonous gas cloud in a Small Splash area, inflicting poison on a 1x [Spirit Power]*10 \% chance. The shaman decides what kind of poisonous effect the gas has. This poison cloud affects enemies and allies alike.
...
The caster creates a sound, dictated by the caster, and throws it forward. Any enemies within Large Splash area from where the sound happened gain a Minor distraction for duration dictated by Spirit Power:
...
\begin{compactitem}
	\item \textbf{ [Armor] Tornado enchantment:} Armor/Clothes causes Minor Distraction on Small Splash Template centered on wearer [Spirit Power] times per day.
