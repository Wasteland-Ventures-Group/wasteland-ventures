...
%Caster teleports [Spirit Power]/2 number allies (to a minimum of 1) \textit{or} self. In combat, the allies have to be in a Tiny Splash Template from the caster, and can be teleported within a Huge Splash Template. Teleported allies can re-appear separate or in a group.
...
The caster sends a ball of lightning that bounces between maximum of [Spirit Power]/2 number targets in a Large Splash Template. This lightning ball can hit a same target multiple times, but not twice in a row. Each hit causes 1+(4) damage to the target.
...
The caster creates a poisonous gas cloud in a Small Splash Template, inflicting poison on a 1x [Spirit Power]*10 \% chance. The shaman decides what kind of poisonous effect the gas has. This poison cloud affects enemies and allies alike.
...
The caster creates a sound, dictated by the caster, and throws it forward. Any enemies within Large Splash Template from where the sound happened gain a Minor distraction for duration dictated by Spirit Power:
...
Caster summons a barrage of barely visible disks to hurl at an enemy. These disks ignore \textit{Dodge-combat action}, and the attack is considered a Burst attack. The hail of discs deals 2+(4) in damage, with Spirit Power determining the amount of additional dice this attack gets:
